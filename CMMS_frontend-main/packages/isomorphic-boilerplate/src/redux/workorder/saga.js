      ...
      callResult.data.data.map((value, index) => {
        ...
        value.completedByUser =
          value.intCompletedByUserID != null
            ...
            : '';
        value.intScheduledMaintenanceID = value.intScheduledMaintenanceID ? value.intScheduledMaintenanceID.toString():null;
        temp.push(value);
      });
...
export function* getWorkOrdersBySMID({ payload }) {
  axios.defaults.headers.get['Authorization'] = localStorage.getItem(
    'id_token'
  );
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/workorder/smId/${payload.id}`
    );
    ...
    if (callResult.response != undefined) {
      ...
      yield put({
        type: actions.GET_WO_BY_SM_ID_REDUCER,
        data: temp,
        msg: null,
      });
    }
...
export function* getPrintWorkOrder({ payload }) {
  axios.defaults.headers.get['Authorization'] = localStorage.getItem(
    'id_token'
  );
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/workorder/print/${payload.id}`
    );
    ...
    if (callResult.response != undefined) {
      ...
      yield put({
        type: actions.PRINT_WORKORDERS_REDUCER,
        workorder: callResult.data.data,
        msg: null,
      });
    }
...
export function* addWorkOrder({ payload }) {
  ...
  try {
    const callResult = yield call(
      onPostCallReqeust,
      payload.sendData,
      `${siteConfig.apiUrl}/workorder/clone`
    );
...
export function* addToWorkOrderTask({ payload }) {
  axios.defaults.headers.post['Authorization'] = localStorage.getItem(
    'id_token'
  );
  try {
    const callResult = yield call(
      ...
      `${siteConfig.apiUrl}/workordertask/addTask/${payload.id}`
    );
    ...
    if (callResult.response != undefined) {
      ...
      notification('success', callResult.data.msg);
      yield put({
        type: actions.ADD_TO_WORKORDER_TASK_SUCCESS,
        data: callResult.data.data,
      });
    }
  } catch (error) {
...
export function* createWorkOrderId({ payload }) {
  ...
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/workorder/numberId`
    );
    if (callResult.response != undefined) {
      notification('error', callResult.response.data.msg);
    } else {    
      yield put({
        type: actions.CREATE_WOID_SUCCESS,
        workorderid: callResult.data.data[0] ? callResult.data.data[0]._id + 1 : null,
      });
    }
  } catch (error) {
...
export default function* rootSaga() {
  yield all([
    ...
    yield takeEvery(actions.GET_PRINT_WORKORDERS, getPrintWorkOrder),
    yield takeEvery(actions.DELETE_WORKORDER, deleteData),
    yield takeEvery(actions.CREATE_WOID, createWorkOrderId),
    yield takeEvery(actions.ADD_TO_WORKORDER_TASK, addToWorkOrderTask),
    yield takeEvery(actions.GET_WO_BY_SM_ID, getWorkOrdersBySMID)
