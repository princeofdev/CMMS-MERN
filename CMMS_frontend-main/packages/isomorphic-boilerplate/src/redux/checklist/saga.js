...
export function* getChecklists({ payload }) {
  ...
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/checklist/charterid/${payload.id}`
    );
    ...
    if(callResult.response!=undefined){
      ...
      callResult.data.data.map((value, index) => {   
       ...
       yield put({
         type: actions.GET_CHECKLIST_REDUCER,
...
export function* getProjectById({payload}) {
  ...
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/checklist/${payload.id}`
    );   
  ...
  try {
    ...
    if(callResult.response!=undefined){
      notification('error',callResult.response.data.msg)     
        ...
        yield put({
          type: actions.GET_CHECKLIST,
          payload: {id:payload.sendData.intCharterId}
        });
    }
  }
  catch (error) {  
...
export function* updateCheckList({payload}) {
...
export function* deleteCheckList({payload}) {
  ...
  try {
    ...
    if(callResult.response!=undefined){
      ...
      notification('success',callResult.data.msg)
      yield put({
        type: actions.DELETE_CHECKLIST_REDUCER,
      });
    }     
...
export default function* rootSaga() {
  yield all([
    yield takeEvery(actions.GET_CHECKLIST,   getChecklists),
    yield takeEvery(actions.ADD_CHECKLIST, addCheckList),
    // yield takeEvery(actions.GET_PROJECT_BY_ID, getProjectById),
    yield takeEvery(actions.UPDATE_CHECKLIST, updateCheckList),
    yield takeEvery(actions.DELETE_CHECKLIST, deleteCheckList),
