...
export function* getChargeDepartment() {
  ...
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/chargedepartment`
    );
    ...
    if (callResult.response != undefined) {
      ...
      yield put({
        type: actions.GET_CHARGE_DEPARTMENT_REDUCER,
        departments: temp, //createDemoData(),
      });
    }
  } catch (error) {
...
export function* addChargeDepartment({ payload }) {
  ...
  try {
    ...
    if (callResult.response != undefined) {
      ...
      yield put({
        type: actions.GET_CHARGE_DEPARTMENT,
...
export function* deleteChargeDepartment({ payload }) {
  ...
  try {
    const callResult = yield call(
      onDeleteCallReqeust,
      `${siteConfig.apiUrl}/chargedepartment/${payload.id}`
    );
    if (callResult.response != undefined) {
      notification('error', callResult.response.data.msg);
    } else {
...
export default function* rootSaga() {
  yield all([
    yield takeEvery(actions.GET_CHARGE_DEPARTMENT, getChargeDepartment),
    yield takeEvery(actions.ADD_CHARGE_DEPARTMENT, addChargeDepartment),
    yield takeEvery(actions.GET_BY_ID, getById),
    yield takeEvery(actions.UPDATE_ASSET_CATEGORY, updateAssetCategory),
    yield takeEvery(actions.DELETE_CHARGE_DEPARTMENT, deleteChargeDepartment),
  ]);
}
