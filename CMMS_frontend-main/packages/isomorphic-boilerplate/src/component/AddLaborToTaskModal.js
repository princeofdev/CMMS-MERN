...
export default function (props) {
  ...
  React.useEffect(() => {
    if (Object.keys(workordertask).length != 0) {
      ...
      setCompletedUserName(
        workordertask.intCompletedByUserID != null
          ? workordertask.intCompletedByUserID.strFullName
          : ''
      );
  ...
  React.useEffect(() => {
    setAssingedUserName(assignedToUserName);
    setCompletedUserName(assignedToUserName);
    setDtmDateCompleted(new Date());
  }, [assignedToUserName]);

  React.useEffect(() => {
    setDblTimeEstimatedHours(estimateTime);
  }, [estimateTime]);
  ...
  const onSave = () => {
    ...
    else sendData.intParentWorkOrderTaskID = intParentWorkOrderTaskID;
