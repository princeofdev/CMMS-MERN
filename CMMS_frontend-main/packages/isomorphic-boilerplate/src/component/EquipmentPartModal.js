...
export default function (props) {
  ...
  React.useEffect(() => {
    let equipments = [];
    assets.map((data) => {
      if (data.intCategoryKind === 2) equipments.push(data);
    });
    setAssetEquipments(equipments);
  }, [assets]);
  ...
  React.useEffect(() => {
    ...
    if (strSearchVal == '') {
      var tree = makeTree(equipments);
      setAssetEquipments(tree);
    } else {
      setAssetEquipments(rawAssets);
  ...
  const rowSelection = {
    ...
    onSelect: (record, selected, selectedRows) => {
      props.selectEquipParentId(record);
      props.onCancel();
    },
  };
