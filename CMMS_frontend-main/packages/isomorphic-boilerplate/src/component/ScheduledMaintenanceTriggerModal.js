...
const [strTSType,setStrTSType]=React.useState('h');
...
React.useEffect(() => { 
    ...
    if(pageState=='edit'){
      ...
      setBolTSWMonday(sheduledTrigger.bolTSWMonday);
      setBolTSWSaturday(sheduledTrigger.bolTSWSaturday);
      setBolTSWSunday(sheduledTrigger.bolTSWSunday);
      setBolTSWThursday(sheduledTrigger.bolTSWThursday);
      setBolTSWTuesday(sheduledTrigger.bolTSWTuesday);
      setBolTSWWednesday(sheduledTrigger.bolTSWWednesday);
      setStrDatLogicHourly(sheduledTrigger.strDatLogicHourly);
      setStrDatLogicDaily(sheduledTrigger.strDatLogicDaily);
      setStrDatLogicMonthly(sheduledTrigger.strDatLogicMonthly);
      setStrDatLogicYearly(sheduledTrigger.strDatLogicYearly);
      setDblRREndBy(sheduledTrigger.dblRREndBy);
      setDblRRStart(sheduledTrigger.dblRRStart);
      setIntAssetEventTypeID(sheduledTrigger.intAssetEventTypeID!=null?sheduledTrigger.intAssetEventTypeID._id:null);
      sheduledTrigger.intAssetEventTypeID!=null?setStrAssetEventTypeName(sheduledTrigger.intAssetEventTypeID.strEventCode+"-"+sheduledTrigger.intAssetEventTypeID.strEventName):setStrAssetEventTypeName('');
      sheduledTrigger.intRMeterReadingUnitID!=null?setIntRMeterReadingUnitID(sheduledTrigger.intRMeterReadingUnitID._id):setIntRMeterReadingUnitID(null);
      setStrType(sheduledTrigger.strType);
      setDblRMeterReading(sheduledTrigger.dblRMeterReading);
      setStrTSType(sheduledTrigger.strTSType);
      setIntTSWEveryWeeks(sheduledTrigger.intTSWEveryWeeks);
      setIntScheduledMaintenanceID(sheduledTrigger.intScheduledMaintenanceID);
      setIntTSDEveryDays(sheduledTrigger.intTSDEveryDays);
      setIntTSHEveryHours(sheduledTrigger.intTSHEveryHours);
      setIntTSMDayOfMonth(sheduledTrigger.intTSMDayOfMonth);
      setIntTSMEveryMonths(sheduledTrigger.intTSMEveryMonths);
      setIntTSWEveryWeeks(sheduledTrigger.intTSWEveryWeeks);
      setIntTSYDayOfMonth(sheduledTrigger.intTSYDayOfMonth);
      setIntTSYEveryYears(sheduledTrigger.intTSYEveryYears);
      setIntTSYMonthOfYear(sheduledTrigger.intTSYMonthOfYear);
      setStrROType(sheduledTrigger.strROType);
      setStrRRType(sheduledTrigger.strRRType);

      setBolNoEndDate(sheduledTrigger.strRRType=="n"?true:false);//strRRType;   bolNoEndDate: setBolEndBy n:b
      setBolEndBy(sheduledTrigger.strRRType=="b"?true:false);

      setStrRType(sheduledTrigger.strRType);
      setBolMTevery(sheduledTrigger.strRType?"e":"o");
      ...
      setStrTRType(sheduledTrigger.strTRType);
      setStrMrLogic(sheduledTrigger.strMrLogic);
      setBolMrByWOClosed(sheduledTrigger.bolMrByWOClosed);
      setBolCreateWorkOrderOnStartDate(sheduledTrigger.bolCreateWorkOrderOnStartDate);
      setStrScheduleDescription(sheduledTrigger.strScheduleDescription);
      setStrScheduleDescription(sheduledTrigger.strScheduleDescription);
      setDatTREndBy(moment(new Date(sheduledTrigger.datTREndBy), 'MMM DD YYYY'));
      setDatTRStart(moment(new Date(sheduledTrigger.datTRStart), 'MMM DD YYYY'));
     //console.log(moment(new Date(), 'MMM DD YYYY'),new Date());
      //setDatTRStart(moment(new Date(), 'MMM DD YYYY'));
      //intAssetID(sheduledTrigger.intAssetID);

      // setDblRRStart(scheduledtrigger.dblRRStart);
      // setStrDatLogicYearly(scheduledtrigger.strDatLogicYearly);

    }
}, [sheduledTrigger,pageState,visible]);
      ...
      if(endDt<=currentDate){          
        ...
        if( strTSType=="h"){
         ...
         slices.splice(slices.length-1,1);
          ...
          if(bolNoEndDate){ // if No end data   
            var endDt=  new Date();
             endDt.setFullYear(endDt.getFullYear() + 5);
             end=endDt;
           }
            console.log(intTSYDayOfMonth, intTSYMonthOfYear, start.getMonth());
            while (end.getTime() >= start.getTime()) {
  ...
  >
    <Row style={rowStyle} gutter={16} justify="start">
      ...
      <Col md={6} sm={6} xs={12}>
        {/* select option */}
        <Select
          defaultValue={intTSYMonthOfYear}
          onChange={(value) => {
            setIntTSYMonthOfYear(value);
          }}
          style={{ width: "100px" }}
        >
    ...
    <Row style={rowStyle} gutter={16} justify="start">
      <Col md={24} sm={24} xs={22}>
        <div style={{ fontStyle: "italic" }}>
          {" "}
          Day and month on work order will automatically adjust if floating
        </div>
      </Col>
    </Row>
    <Row style={rowStyle} gutter={16} justify="start">
      <Col md={6} sm={6} xs={24}>
        <RadioGroup
          onChange={(event) => setStrDatLogicYearly(event.target.value)}
          name="value"
          value={strDatLogicYearly}
        >
          <Radio style={radioStyle} value={"x"}>
            Fixed
          </Radio>
      ...
      <Col md={18} sm={18} xs={24}>
        ...
        >
          {strDatLogicMonthly == "x" ? (
            <span>
              By setting a fixed date trigger, you are indicating you wish the
              next trigger date to always occur on the date triggers you have
              set.
            </span>
          ) : (
            <span>
              By setting a floating date trigger, you are indicating you wish
              the next trigger date to be determined based on the trigger period
              and the close date of the created Work Order.
            </span>
          )}
        </div>
      </Col>
...
const sectionPart=()=>{
 ...
 return <div style={{paddingLeft:"20px",paddingRight:"20px"}}>
              <div  style={{borderRadius: "4px",border:"1px solid rgb(205, 209, 215)",padding:"4px", margin:"4px"}} >
                    <Row style={rowStyle} gutter={16} justify="start">
                      <Col md={2} sm={2} xs={2}>
                         <Radio style={radioStyle}  checked={bolMTevery} onClick={()=>{setBolMTevery(!bolMTevery);setBolMTwhen(bolMTevery)}}>
                                 Every
                         </Radio>
                      </Col>
                      <Col md={4} sm={4} xs={24} > 
                          <Form>    
                             <Input placeholder="" value={dblRMeterReading} onChange={(event)=>setDblRMeterReading(event.target.value)} style={{width:"100px"}}/> 
                          </Form>   
                      </Col>
                      <Col md={12} sm={12} xs={24} >                             
                          <Select
                                    ...
                                    onChange={(value)=>{setIntRMeterReadingUnitID(value)}}
                                    style={{ width: '250px' }}
                                  >
                                     {
                                      meterreadingunits.map((row)=>{
                    ...
                    <Row style={rowStyle1} gutter={16} justify="start">
                      <Col md={6} sm={6} xs={24} > 
                          <Form>
                            <FormItem {...formItemCode}  label="Start At ">
                             <Input placeholder=""  value={dblRRStart} onChange={(event)=>setDblRRStart(event.target.value)}
                             />                                 
                            </FormItem>
                          </Form>   
                      </Col>
                      <Col md={5} sm={5} xs={24} >
                            <RadioGroup onChange={(event)=>setStrTRType(event.target.value)} value={strTRType}>
                            ...
                            <Input placeholder="" value={dblRREndBy} onChange={(event)=>setDblRREndBy(event.target.value)} style={{width:"60px"}}/> 

                      </Col>
                      <Col md={8} sm={8} xs={24} >
                           <div style={{background:"#efefef",borderRadius: "3px",marginLeft: "20px",padding: "5px 10px 10px 10px"}}>
                            <RadioGroup onChange={onChangeStrMrLogic} name="value" value={strMrLogic}>
                                  <Radio style={radioStyle} value={"x"}>
                                      Fixed Schedule
                                  </Radio>
                                  <Radio style={radioStyle} value={"t"}>
                            ...
                            <Checkbox checked={bolMrByWOClosed} onChange={(event)=>setBolMrByWOClosed(event.target.checked)} >By Work Order Closed </Checkbox> 
                             <Tooltip
                                title="This setting ensures that the next trigger threshold should reflect a floating schedule based on the meter reading at the time of closing the work order, and not the meter reading when the work order is created."
                              >
                                <span>
                                 (?)
                                </span>
                      ...
                      <Col md={5} sm={5} xs={24} >
                           <div style={{background:"#efefef",borderRadius: "3px",marginLeft: "20px",padding: "5px 10px 10px 10px"}}>
                              Next Trigger<br></br>
                              {parseFloat(dblRMeterReading)+parseFloat(dblRRStart)}
                            </div>
                      </Col>
                      </Row>                   
                    </div>
                    <div  style={{borderRadius: "4px",border:"1px solid rgb(205, 209, 215)",padding:"4px", margin:"10px 4px 4px 4px"}} >
                    <Row style={rowStyle} gutter={16} justify="start">
                      <Col md={3} sm={3} xs={3}>
                         <Radio style={radioStyle} checked={bolMTwhen} onClick={()=>{setBolMTwhen(!bolMTwhen);setBolMTevery(bolMTwhen)}}>
                             When 
                         </Radio>
                      </Col>
                      <Col md={10} sm={10} xs={20}>
                          <Select
                                    defaultValue={3}
                                    onChange={(value)=>setIntROMeterReadingUnitID(value)}
                      ...
                      <Col md={8} sm={8} xs={24}>
                            <Select
                                    defaultValue={strROType}
                                     onChange={(value)=>{setStrROType(value);}}
                                    style={{ width: '100%' }}
                                  >
                                    <Option value="l">Reading is Less than</Option>
                                    <Option value="g">Reading is Greater than</Option>                
                                   
                            </Select>
                      </Col>
                      <Col md={3} sm={3} xs={24}>
                            <Form>    
                             <Input placeholder="" value={dblROMeterReading} onChange={(event)=>setDblROMeterReading(event.target.value)} style={{width:"70px"}}/> 
                          </Form> 
                      </Col>
                      </Row>
                    </div>
              ...
              <div  style={{borderRadius: "4px",border:"1px solid rgb(205, 209, 215)",padding:"4px", margin:"4px"}} >
                    <Row style={rowStyle} gutter={16} justify="start">                        
                        <Col md={7} sm={7} xs={24}>
                            <Form>
                              <Fieldset>
                               <span>Every Occurrance Of Event</span>
                               </Fieldset>
                            </Form>                            
                        </Col>
                        <Col md={12} sm={12} xs={24}>                        
                            ...
                            <div style={{ position: "relative" }}>
                             ...
                             onClick={()=>{setAssetEventModalActive(true)}}
                                style={{
                                  fontSize: "25px",
                                  cursor: "pointer",
                                  position: "absolute",
      ...
      <Row style={rowStyle} gutter={16} justify="start">
          <Col md={24} sm={24} xs={24} >  
             <RadioGroup onChange={onChange} name="value" value={strType}>
                    ...
                    <Radio style={radioStyle} value={'r'}>
                         Generate Work Order By Meter Reading Trigger
                    </Radio>
                    <Radio style={radioStyle} value={'e'}>
                         Generate Work Order By Event Trigger
                    </Radio>
              </RadioGroup>
          </Col>
      </Row>
