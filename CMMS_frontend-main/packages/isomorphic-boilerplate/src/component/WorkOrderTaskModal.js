...
export default function (props) {
  ...
  const [
    completedUserModalActive,
    setCompletedUserModalActive,
  ...
  React.useEffect(() => {
    if (Object.keys(workordertask).length != 0) {
      ...
      setIntAssignedToUserID(
        workordertask.intAssignedToUserID != null
          ? workordertask.intAssignedToUserID._id
          : null
      );
      setAssingedUserName(
        workordertask.intAssignedToUserID != null
          ? workordertask.intAssignedToUserID.strFullName
          : ''
      );
      setIntCompletedByUserID(
        workordertask.intCompletedByUserID != null
          ? workordertask.intCompletedByUserID._id
          : null
      );
  ...
  const onSave = () => {
    ...
    var sendData = {};
    if (workorderId === null) sendData.intWorkOrderID = addWorkorderId;
    else sendData.intWorkOrderID = workorderId;
    ...
    sendData.strAssets = assetName;
    if (intTaskType == 3) {
      if (!bolPassNotify && !bolFailNotify) sendData.strResult = '';
      else {
    ...
    if (assetId == 'All') sendData.intAssetID = strAssetIds;
    else sendData.intAssetID = assetId;

    if (pageState == 'edit') dispatch(updateData(sendData, intWorkOrderTaskId));
    else dispatch(addWorkOrderTask(sendData));
    ...
    if (intTaskType == 3 && bolCreateWorkOrder) {
      ...
      workOrder.strAssignedUsers = workorder.strAssignedUsers;
      workOrder.intAssignedUserId = workorder.intAssignedUserId;
      workOrder.intEstimatedHour = workorder.intEstimatedHour;
      workOrder.strWorkInstruction = workorder.strWorkInstruction;
      workOrder.strDescription = strDescription;
      workOrder.strAssetIds = workorder.strAssetIds;
      workOrder.strAssets = workorder.strAssets;
      workOrder.dtmSuggestedCompletionDate =
        workorder.intEstimatedHour > 8
          ? moment(new Date()).add('days', 1).format()
        ...
        <Row style={rowStyle} gutter={16} justify="start">
          ...
          <Col md={14} sm={14} xs={24} style={{ marginBottom: '2px' }}>
            <DatePicker
              ...
              value={
                dtmStartDate != null
                  ? moment(dtmStartDate, 'YYYY-MM-DD HH:mm:ss')
                  : ''
              }
              onChange={onChange}
              style={{ width: '70%' }}
          ...
          <Col md={14} sm={14} xs={24} style={{ marginBottom: '2px' }}>
            <div style={{ position: 'relative' }}>
              ...
              <i
                className="ionicons ion-arrow-down-b"
                onClick={() => {
                  setCompletedUserModalActive(true);
                }}
                style={{
                  fontSize: '25px',
                  cursor: 'pointer',
        ...
        <Row style={rowStyle} gutter={16} justify="start">
          ...
          <Col md={14} sm={14} xs={24} style={{ marginBottom: '2px' }}>
            <DatePicker
              showTime
              value={
                dtmDateCompleted != null
                  ? moment(dtmDateCompleted, 'YYYY-MM-DD HH:mm:ss')
                  : ''
              }
      ...
      <div>
        ...
        <Row style={rowStyle} gutter={16} justify="start">
          <Col
            ...
            sm={10}
            xs={24}
            style={{ marginBottom: '2px', paddingLeft: '25px' }}
          >
            <Checkbox
              checked={bolPassNotify}
              onChange={(event) => {
                setBolPassNotify(event.target.checked);
                setBolFailNotify(event.target.checked ? false : false);
              }}
            >
              Notify Users
            </Checkbox>
        ...
        <Row style={rowStyle} gutter={16} justify="start">
          ...
          >
            <Checkbox
              checked={bolFailNotify}
              onChange={(event) => {
                setBolFailNotify(event.target.checked);
                setBolPassNotify(event.target.checked ? false : false);
              }}
            >
              Notify Users
            </Checkbox>
        ...
        <Row style={rowStyle} gutter={16} justify="start">
          ...
          >
            <Checkbox
              checked={bolCreateWorkOrder}
              onChange={(event) => setBolCreateWorkOrder(event.target.checked)}
            >
              Create a Follow-on Work Order{' '}
            </Checkbox>
          </Col>
        </Row>
        {bolCreateWorkOrder ? (
