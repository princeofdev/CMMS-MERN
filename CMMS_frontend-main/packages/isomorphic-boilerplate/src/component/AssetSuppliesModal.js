...
const { getAllAsset, getSupplies} = assetActions;

export default function (props) {
  const { visible, title, filter} = props;
  ...
  const [intFilterId,setFilterId]=React.useState(1);
  const rowStyle = {
    width: "100%",
    display: "flex",
    flexFlow: "row wrap",
  ...
  const columns = [
    {
      ...
      render: (text, row) => {
        ...
        return row.intCategoryKind == 1 ?<span> <img style={{ width: "16px", height: "17px", marginRight: "5px" }} src={treeImg}></img>{text}</span>:<a onClick={() => {
          selectAsset(row);
        }}> <img style={{ width: "16px", height: "17px", marginRight: "5px" }} src={treeImg}></img>{text}</a>
      },
    },
    {
      title: "Name",
      ...
      render: (text, row) => {
        ...
        return row.intCategoryKind == 1 ? <span >{text}</span>:<a onClick={() => {
          selectAsset(row);
        }} >{text}</a>
      }
      ,
    },
    {
      ...
      rowKey: "strCode",
      width: "15%",
      render: (text, row) => row.intCategoryKind == 1 ?<span>{text}</span>:<a onClick={() => {
        selectAsset(row);
      }}>{text}</a>,
    },
    {
...
//  if(visible==true)
   ...
   setFilterId(filter =='Facilities'?1:2);
}, [visible]);
React.useEffect(() => {
  
   if(strSearchVal=="" && visible==true){
