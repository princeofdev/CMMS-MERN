...
export default function Audits() {
  ...
  const { entriesLists } = useSelector((state) => state.EntriesCrew);
  const { users } = useSelector((state) => state.Users);
  const dispatch = useDispatch();
  const match = useRouteMatch();
  const [entryCrewId, setEntryCrewId] = useState();

  const goAdd = () => {
    const id = entryCrewId;
    history.push(`/dashboard/entries_crew/add/${id}`);
  };
  ...
  React.useEffect(() => {
    if (entriesLists.length !== 0) setEntryCrewId(entriesLists.length);
  }, [entriesLists]);
    ...
    },
    {
      ...
      render: (text, row) => {
        ...
        return (
          <a onClick={() => goDetail(row._id)}>
            {printedDate != null
              ? moment(printedDate).format('YYYY-MM-DD')
              : ''}
          </a>
        );
      },
  ...
  return (
    <LayoutWrapper>
      <PageHeader>
        <IntlMessages id="sidebar.entries_crew" />
      </PageHeader>
      <Box>
