...
export default function (props) {
  ...
  const { getFilesByPurchaseOrder } = FilesAction;
  const { fileLists } = useSelector((state) => state.Files);
  const [data, setData] = React.useState([]);
  const [fileUploadModalActive, setFileUploadModalActive] = React.useState(
    false
  );
  const columns = [
    {
  ...
  const loadSuccess = () => {
    dispatch(getFilesByPurchaseOrder(props.purcaseOrderId));
    setFileUploadModalActive(false);
  };
  React.useEffect(() => {
    dispatch(getFilesByPurchaseOrder(props.purcaseOrderId));
    ...
    <div className="isoInvoiceTable">
      ...
      >
        ...
        >
          <img
            src={newAddImg}
            onClick={() => setFileUploadModalActive(true)}
          ></img>
        </span>
      </div>
      <FileUploadModal
        ...
        intPurchaseOrderID={props.purcaseOrderId}
        title="Upload file"
