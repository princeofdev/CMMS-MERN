...
import businessesAction from '../../redux/businesses/actions';
import fakeData from './data';

const dataList = new fakeData(10);

const { getBusinessData } = businessesAction;
...
export default function Businesses() {
  ...
  const { businesses, isDelete } = useSelector((state) => state.Businesses);
  const goDetail = (id) => {
    history.push(`/dashboard/businesses/edit/${id}`);
  };
  ...
  // React.useEffect(() => {
  //   let tmp = businesses.map((item,index) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'strBusinessClassification',
      key: 'strBusinessClassification',
      // width: "10%",
      render: (text, row) => {
        return (
          <a
