...
import ScheduledNotification from '../../../component/ScheduledNotificationModal';
import ScheduleNotificationAction from '../../../redux/schedulednotification/actions';
export default function(props) {   
  const dispatch = useDispatch();
  const { smId}=props;
   const [data, setData] = React.useState([]);
  const { getDatas } = ScheduleNotificationAction;
  const { schedulenotifications, isDelete } = useSelector(
   ...
   const columns = [
    {
      ...
      width: "40%",
      render: (text,row) => {      
        return <a onClick={() => goDetail(row)}>{text?text.strFullName:''}</a>;
     },
    },
  ...
  const goDetail = (row) => {
    setSchedulenotification(row);
    setPageState('edit');   
    setNotificationModalActive(true);
  };
  React.useEffect(() => {
    dispatch(getDatas(smId));
  }, []);
  ...
  return (
    <div className="isoInvoiceTable">
      ...
      > */}
        ...
        dataSource={schedulenotifications}
          columns={columns}
          pagination={false}
      ...
      <div style={{color: "rgb(102, 115, 136)",   
          ...
          height: "25px"}}>
        <span onClick={() => {setNotificationModalActive(true);setPageState("ADD");}}  style={{float: "left",
                      marginLeft:"4px", 
                    marginRight:"4px",
                    cursor:"pointer",
                    }}>
      ...
      <ScheduledNotification
        ...
        scheduleNotificaton={schedulenotification}
        pageState={pageState}
        intScheduledMaintenanceID={smId}
        title="SCHEDULED MAINTENANCE USER"
      >
