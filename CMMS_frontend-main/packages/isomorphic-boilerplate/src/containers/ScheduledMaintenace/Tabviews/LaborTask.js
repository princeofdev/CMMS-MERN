...
export default function (props) {
  ...
  const { getDatas, deleteData } = ScheduledTaskAction;
  const [
    scheduledTaskModalActive,
    setScheduledTaskModalActive,
  ...
  const { scheduledtasks, isDelete } = useSelector(
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, row, index) => {
        let nextRow = scheduledtasks[index + 1];
        let nextRowId =
          nextRow != undefined ? nextRow.intParentScheduledTaskID : null;
        if (row.intTaskType == 3) {
          return row.strResult == 'Pass' ? (
          ...
          return (
            <RadioGroup name="value" value={'Pass'}>
              <Row gutter={16} justify="start">
                <Col md={12} sm={12} xs={24}>
                  <Radio style={radioStyle} value={'Pass'}>
                    Pass
  ...
  React.useEffect(() => {
    if (isDelete) dispatch(getDatas(scheduledMaintenaceId));
  }, [isDelete]);
  ...
  return (
    <div className="isoInvoiceTable">
      ...
      <ScheduledTaskModal
        ...
        assetId={assetId}
        scheduledmaintenance={props.scheduledmaintenance}
