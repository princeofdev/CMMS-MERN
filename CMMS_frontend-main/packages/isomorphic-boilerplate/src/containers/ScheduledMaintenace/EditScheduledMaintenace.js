...
const { addWorkOrder, addToWorkOrderTask } = workorderAction;
...
export default function (props) {
  ...
  const { scheduledtasks } = useSelector((state) => state.ScheduledTask);
  ...
  const [addWorkorderId, setAddWorkorderId] = React.useState(0);
  ...
  const onPDF = () => {
    ...
    history.push(`/dashboard/scheduledmaintenance/print/${id}`);
  };
  ...
  const onCreateWorkOrder = () => {
    // dispatch(addToWorkOrderTask(scheduledtasks, addWorkorderId));
    var sendData = {
      intPriorityID: intPriorityID,
      ...
      strCompletionNotes: strCompletionNotes,
      intMaintenanceTypeID: intMaintenanceTypeID,
      dtmEstimatedStartDate: moment().format(),
      ...
      intAssignedUserId: intAssignedToUserID,
      intScheduledMaintenanceID: id,
    };

    console.log(sendData);
    dispatch(addWorkOrder(sendData));
  };
  const scheduledTaskData = (scheduledtasks) => {
                        ...
                        <Form>
                          <Fieldset>
                            ...
                            <FormItem
                              hasFeedback
                              validateStatus={
                                strCode == '' ? 'error' : 'success'
                              }
                              help={
                                strCode == '' ? 'this field is require' : ''
                              }
                            >
                              <Input
                                label="Code"
