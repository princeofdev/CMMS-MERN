...
export default function (props) {
  ...
  const onDateChange = (value, dateString) => {
    ...
    props.assignCompletionDate(dateString);
  };
  React.useEffect(() => {
    if (data != null) {
      setAssignCompletionDate(data.aCompletionDate);
      setStrNarrative(data.strNarrative);
      setStrHowImproveFuture(data.strImproveFuture);
    }
  }, [data]);
                  ...
                  <Form>
                    <Fieldset>
                      ...
                      <Textarea
                        ...
                        value={strNarrative}
                        onChange={(event) => {
                          setStrNarrative(event.target.value);
                          props.strNarrative(event.target.value);
                        }}
                      />
                    </Fieldset>
                  </Form>
                  ...
                  <Form>
                    <Fieldset>
                      ...
                      <Textarea
                        ...
                        style={{ height: 'auto' }}
                        rows={3}
                        value={strHowImproveFuture}
                        onChange={(event) => {
                          setStrHowImproveFuture(event.target.value);
                          props.strHowImproveFuture(event.target.value);
                        }}
                      />
                    </Fieldset>
                  </Form>
