...
import { NCR, Report, Plan, Inspection, Nonconformities } from './Tabviews/Tabviews';
...
export default function (props) {
  ...
  const [
    strPlanOpenMeetingPresent,
    setStrPlanOpenMeetingPresent,
  ] = React.useState('');
  const [strNonConformity, setStrNonConformity] = React.useState('');
  const [
    strPlanOpenMeetingDetail,
    setStrPlanOpenMeetingDetail,
  ] = React.useState('');
  const [
    strPlanConductAuditBegin,
    setStrPlanConductAuditBegin,
  ] = React.useState('');
  const [
    strPlanConductAuditClose,
    setStrPlanConductAuditClose,
  ] = React.useState('');
  const [
    strPlanConductAuditDetail,
    setStrPlanConductAuditDetail,
  ] = React.useState('');
  const [
    strPlanCloseMeetingBegin,
    setStrPlanCloseMeetingBegin,
  ] = React.useState('');
  const [
    strPlanCloseMeetingClose,
    setStrPlanCloseMeetingClose,
  ] = React.useState('');
  const [
    strPlanCloseMeetingPresent,
    setStrPlanCloseMeetingPresent,
  ] = React.useState('');
  const [
    strPlanCloseMeetingDetail,
    setStrPlanCloseMeetingDetail,
  ] = React.useState('');
  ...
  const [
    strReportImmediateAction,
    setStrReportImmediateAction,
  ] = React.useState('');
  const [
    strReportImmediateCompletionDate,
    setStrReportImmediateCompletionDate,
  ] = React.useState('');
  const [strReportFurtherAction, setStrReportFurtherAction] = React.useState(
    ''
  );
  const [
    strReportFurtherCompletionDate,
    setStrReportFurtherCompletionDate,
  ] = React.useState('');
  const [strReportFollowUpDetail, setStrReportFollowUpDetail] = React.useState(
    ''
  );
  const [
    strReportCorrectiveAction,
    setStrReportCorrectiveAction,
  ] = React.useState('');
  ...
  const [
    strNCRImmediatePersonName,
    setStrNCRImmediatePersonName,
  ] = React.useState('');
  const [
    strNCRImmediateCompetionDate,
    setStrNCRImmediateCompetionDate,
  ] = React.useState('');
  ...
  const [
    strNCRFurtherCompetionDate,
    setStrNCRFurtherCompetionDate,
  ] = React.useState('');
  const [
    strNCRVerificationCorrectiveAction,
    setStrNCRVerificationCorrectiveAction,
  ] = React.useState('');
  const [strNCRFollowUpDetail, setStrNCRFollowUpDetail] = React.useState('');
  const [
    strNCRCorrectiveActionClose,
    setStrNCRCorrectiveActionClose,
  ] = React.useState('');

  const [nonconformitiesData,setNonconformitiesData]=React.useState(null);
  ...
  const NonconformitiesInfo = (info) => {
    setNonconformitiesData(info);
  }
  ...
  const NCRInfo = (info) => {
    ...
    setStrNCRVerificationCorrectiveAction(
      info.strNCRVerificationCorrectiveAction
    );
  ...
  const onUpdate = (status) => {
    var sendData = {
      ...
      strTitle: strTitle,
      strNonConformity: strNonConformity,
      strSMSReference: strSMSReference,
      strISMReference: strISMReference,
      NCR:{
        strInternalSmsAuditReport: nonconformitiesData.strInternalSmsAuditReport,
        aDate: nonconformitiesData.aDate,
        strDepartmentVessel: nonconformitiesData.strDepartmentVessel,
        strAuditNo: nonconformitiesData.strAuditNo,
        strAuditor: nonconformitiesData.strAuditor,
        strNCRCARNo: nonconformitiesData.strNCRCARNo,
        strAuditorAuditee: nonconformitiesData.strAuditorAuditee,
        strISMReference: nonconformitiesData.strISMReference,
        strSMSReference: nonconformitiesData.strSMSReference,
        strNCStatement: nonconformitiesData.strNCStatement,
        strImmediateAction: nonconformitiesData.strImmediateAction,
        aImmediateCompletionDate: nonconformitiesData.aImmediateCompletionDate,
        strFurtherAction: nonconformitiesData.strFurtherAction,
        aFurtherCompletionDate: nonconformitiesData.aFurtherCompletionDate,
        strFollowUpDetail: nonconformitiesData.strFollowUpDetail,
        strCorrectiveAction: nonconformitiesData.strCorrectiveAction,
      }
    };
  ...
  const changePassState = (state) => {
    ...
    if (state == 'fail') setStrStatus('NCR');
    else setStrStatus('complete');
  };
  const onDelete = () => {
  ...
  React.useEffect(() => {
    ...
    if (Object.keys(audit).length !== 0) {
      ...
      setStrNonConformity(audit.audit.strNonConformity);
      setNonconformitiesData(audit.audit.Nonconformities);
    }
  }, [audit]);
      ...
      <TableTabsStyle className="isoLayoutContentAsset">
                ...
                <Form>
                  <Fieldset>
                    ...
                    <Textarea placeholder=""
                      style={{ height: 'auto' }}
                      rows={3}
                      value={strNonConformity}
                      name="Non-conformity"
                      onChange={(event) => { setStrNonConformity(event.target.value) }}
                    />
                  </Fieldset>
                </Form>
              </Col>
        ...
        >
          ...
          <TabPane tab="Audit Plan" key="2">
            <Plan data={aPlan} planInfo={planInfo}></Plan>
          </TabPane>
          ...
          {strPassState == 'fail' && (
            <TabPane tab="NCR/CAR" key="4">
              <NCR data={aNCRCAR} NCRInfo={NCRInfo}></NCR>
            </TabPane>
          )}
          <TabPane tab="Nonconformities" key="5">
            <Nonconformities
            pageState={"Edit"}
              data={nonconformitiesData}
              NonconformitiesInfo={NonconformitiesInfo}
            ></Nonconformities>
          </TabPane>
        </Tabs>
      </TableTabsStyle>
