...
export default function Assets() {
  ...
  const [isToplovel, setIsToplovel]=React.useState(false);
  let isVessel=false;
  ...
  const columns1 = [
    {
      ...
      render: (text, row) => {  
        ...
        if (row.intCategoryKind === 1) {         
          ...
          assetKind = 'equipment';
        } else if (row.intCategoryKind === 3) {        
          assetKind = 'tool';
        }
        else if (row.intCategoryKind === 0) {        
          assetKind = 'vessel';
        }
        return (
          <span>           
            <a onClick={() => goDetail(assetKind, row._id)}>{text}</a>
          </span>
        );
      },
    },
      ...
      render: (text, row) => {
        ...
        return (
          <span>
            ...
            {
              treeImg ? <img
                style={{ width: '16px', height: '17px', marginRight: '5px' }}
                src={treeImg}
              ></img>:null
            }            
            <a onClick={() => goDetail(assetKind, row._id)}>{text}</a>
  ...
  const goAssets = (id) => {
    if (id == 1) {
      ...
      if(isToplovel){
        notification('info', 'Top level vessel already exists');
        return;
      }
      history.push('/dashboard/asset/add/vessel');
    }
    //setModalVisible(false);
  };
  const makeTree = (arr) => {
    ...
    // topLevel.key='top';
    // topLevel._id='top';
    // topLevel.strCode = "Vessel";
    // topLevel.intCategoryKind = 0;
    // topLevel.strName = "Vessel";
    // topLevel.children=[];
    // topLevel.bolIsOnline=true;

    // First map the nodes of the array to an object -> create a hash table.
    for (var i = 0, len = arr.length; i < len; i++) {
      if (arr[i].intCategoryKind!=0){
        arrElem = arr[i];
    ...
    console.log(topLevel, 'topLevel', isVessel);
    if(isVessel){
      topLevel.children = tree;
      allTree.push(topLevel);
      return allTree;
    }
    else{
      return tree;
    ...
    <LayoutWrapper>
      ...
      <Box>
        ...
        <div className="isoInvoiceTableBtn">
          ...
          <div
            style={{ float: 'left', marginLeft: '10px', paddingTop: '10px' }}
          >
            <div
              style={{ float: 'left', cursor: 'pointer' }}
              onClick={() => {
                setViewOption(1);
                setFilterName('filter by Category : Assets');
              }}
            >
